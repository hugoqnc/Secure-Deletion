...
Additionally, we defined a $i$-versioned term as a versioned term that is accessible in version $i$.
...
\subsubsection{Developer interest}
\label{sec:developer-interest}
...
It is therefore their in their interest to carefully choose permission amounts.
...
This value is used to specify the amount of permission of \texttt{guard} to consume (line 5), as discussed in section \ref{sec:developer-interest}.
...
But as discussed in section \ref{sec:developer-interest}, it is in the developer's interest to specify the full amount if they want verification to succeed.
...
As explained before, we do not enforce the verification that the decryption key is versioned. In a similar manner as in section \ref{sec:developer-interest}, we can prove that this does not impact soundness and it is in the developer's interest to use a versioned decryption key in this case.
