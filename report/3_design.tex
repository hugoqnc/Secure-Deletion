...
We start by explaining the goal of our methodology by introducing a relevant protocol, the Diffie-Hellman Ratchet, and the security properties we want to prove about it.
...
In the DH Ratchet protocol, we consider that the attacker compromised Alice's $K_n$ communication key \emph{after} she derived the new $K_{n+1}$ key and deleted the DH private key she used to compute $K_{n+1}$.
At this point, the attacker cannot obtain $K_{n+1}$ because they cannot have access to the new DH shared secret.
...
This solution is however not applicable to us because, as explained previously, we do not restrict the way implementations store application state, and we use a ghost trace, removed at runtime, that cannot store program data.
...
Instead, to solve this problem without the limitations of a counter variable, we will create a mechanism based on \emph{counting permissions}. Those will be explained in the next section, before using them to design our deletion mechanism.
...
Gobra does not use counting permissions and relies only on the fractional permissions model, which was explained in section \ref{sec:permissions}.
...
func (l *LabeledLibrary) CreateNonce(ghost nonceLabel SecrecyLabel,
    ...
    CanFlow represents the flowing relation between secrecy labels, as defined in section \ref{sec:dy-star}, and takes a snapshot of the global trace and the two secrecy labels as arguments.}
