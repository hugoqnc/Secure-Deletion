...
\todo{Explain how we can express security properties using versions}
% Similarly to how DY* verifies forward secrecy and post-compromise security for the Signal protocol, we could express these two properties as a single one.


% Suppose a participant $p$ executes the ratcheting protocol in session $s$ with a peer participant $p'$, and suppose that $s$ stores a communication key $K_{n+1}$ in the current version $v$.
% There must exist a previous version $v'<v$ when $K_n$ was stored in $s$ but $K_{n+1}$ remained secret, unless either $K_n$ becomes public before the session's version reaches $v$ or 
