...
// dst == kdfRatchet(K, DHSharedSecret) && len(dst) == 32
//@ trusted
//@ requires Mem(dst) && acc(Mem(K), 1/16) && acc(Mem(DHSharedSecret), 1/16)
//@ requires Size(dst) == 32 && Size(K) == 32
//@ requires versionPerm > 0 && acc(guard(currentVersion), versionPerm)
//@ ensures  Mem(dst) && acc(Mem(K), 1/16) && acc(Mem(DHSharedSecret), 1/16)
//@ ensures  Size(dst) == 32
//@ ensures  Abs(dst) == by.kdfRatchetB(Abs(K), Abs(DHSharedSecret))
//@ ensures  acc(receipt(dst, currentVersion), versionPerm)
func ComputeKDFRatchet(dst, K, DHSharedSecret ByteString /*@, ghost currentVersion uint32, ghost versionPerm perm @*/) { // TODO_ it is not safe to use currentVersion as a parameter here: we should read it from the initiator state
	device.KDFRatchetSlice(dst, K, DHSharedSecret)
}
...
//@ requires acc(Mem(message.Data), 1/16) && Size(message.Data) == DHHalfKeyLength
//@ ensures  acc(message, 1/16) && acc(Mem(message.Payload), 1/16) &&  acc(Mem(message.Data), 1/16) && Mem(res) && Size(res) == Size(message.Payload) + DHHalfKeyLength+16
//@ ensures  Abs(res) == by.tuple5B(by.integer32B(message.Type),by.integer32B(message.Receiver),by.integer64B(message.Nonce),Abs(message.Data),Abs(message.Payload))
func MarshalMessage(message *Message) (res ByteString) {
	...
	fieldData := buff[16:DHHalfKeyLength+16]
	...
	copy(fieldData, message.Data)
	copy(buff[DHHalfKeyLength+16:], message.Payload)
...
//@ requires acc(Mem(packet), 1/16)
//@ ensures  acc(Mem(packet), 1/16)
//@ ensures  ok ==> Size(packet) >= (DHHalfKeyLength+16) && acc(message) && Mem(message.Payload) && Mem(message.Data) && Size(message.Payload) == Size(packet) - (DHHalfKeyLength+16) && Size(message.Data) == DHHalfKeyLength // TODO_ added trusted size of the message.Data, is it fine?
//@ ensures  ok ==> Abs(packet) == by.tuple5B(by.integer32B(message.Type),by.integer32B(message.Receiver),by.integer64B(message.Nonce),Abs(message.Data),Abs(message.Payload))
func UnmarshalMessage(packet ByteString) (message *Message, ok bool) {
	...
	nonce := binary.LittleEndian.Uint64(
		packet[device.MessageTransportOffsetCounter:device.MessageTransportOffsetData],
	)
	...
	message = &Message{
		...
		Receiver: receiver,
		Nonce:    nonce,
		Data:     make([]byte, DHHalfKeyLength),
		Payload:  make([]byte, len(packet)-device.MessageTransportOffsetContent),
	}

	copy(message.Data, packet[device.MessageTransportOffsetData:device.MessageTransportOffsetContent])
